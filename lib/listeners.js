modtoggle.init = () => {
    ...
    modtoggle.handleEvent = (eventID, ...vararg) =>
        Object.values(modtoggle.events[eventID]).forEach((handle) =>
            handle(...vararg),
        );
};

modtoggle.registerToggle = (toggle) => {
    if (activeToggles[toggle.name])
        modtoggle.toggles[toggle.name].onDeactivate();
    modtoggle.toggles[toggle.name] = toggle;
    if (activeToggles[toggle.name]) modtoggle.toggles[toggle.name].onActivate();
};
...
modtoggle.registerListener = (event, listener, listenerID) => {
    if (modtoggle.eventID[event] == undefined) {
        ...
        let runnable = Core.runnable(
            ...
            `function onEvent(...vararg) { modtoggle.handleEvent("${id}", ...vararg); }`,
        );
