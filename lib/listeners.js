...
modtoggle.handleEvent = function (eventID, ...vararg) {
    for (let [name, handle] of Object.entries(modtoggle.events[eventID])) {
        try {
            let res = handle.apply(null, vararg);
...
modtoggle.registerListener = (event, listener, listenerID, core = Core) => {
